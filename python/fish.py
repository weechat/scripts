#
# SPDX-FileCopyrightText: 2011-2026 David Flatz <david@upcs.at>
# SPDX-FileCopyrightText: 2017-2020 Marcin Kurczewski <rr-@sakuya.pl>
# SPDX-FileCopyrightText: 2017 Ricardo Ferreira <ricardo.sff@goatse.cx>
# SPDX-FileCopyrightText: 2014 Charles Franklin <jakhead@gmail.com>
# SPDX-FileCopyrightText: 2012 Markus Näsman <markus@botten.org>
# SPDX-FileCopyrightText: 2009 Bjorn Edstrom <be@bjrn.se>
#
...
def fish_modifier_input_text(data, modifier, server_name, string):
    ...
    return fish_msg_w_marker(string.encode()).decode()
...
def fish_cmd_blowkey(data, buffer, args):
    ...
    if argv[0] == "genkey":
        if fish_secure_key():
            ...
            return weechat.WEECHAT_RC_ERROR
        new_key = blowcrypt_b64encode(urandom(32))
        weechat.command(
            buffer, f'/set {CONFIG_FILE_NAME}.secure.key ${{sec.data.fish}}')
        weechat.command(buffer, f'/secure set fish {new_key}')
...
def fish_msg_w_marker(msg):
    ...
    if weechat.config_string(fish_config_option["mark_position"]) == "begin":
