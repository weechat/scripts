...
def urlserver_server_reply(conn, code, extra_header, message, mimetype='text/html'):
    ...
    conn.send('HTTP/1.1 %s\r\n'
              ...
              'Content-Length: %d\r\n'
              '\r\n'
              '%s' % (code, extra_header, mimetype, len(message), message))

def urlserver_server_favicon():
    """Return favicon for HTML page."""
    return base64.b64decode(
        'iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAABmJLR0QA/wD/AP+gvaeTAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAB3RJTUUH1wgcDwo4MOEy+AAAAB10'
...
def urlserver_server_reply_list(conn, sort='-time'):
    ...
    if not sort.startswith('-'):
        sort = '+%s' % sort
    if sort[1:] == 'time':
        urls = sorted(urlserver['urls'].iteritems())
    else:
        idx = ['time', 'nick', 'buffer'].index(sort[1:])
        urls = sorted(urlserver['urls'].iteritems(), key=lambda(k,v):v[idx].lower())
    if sort.startswith('-'):
        urls.reverse()
    sortkey = { '-': ('', '&uarr;'), '+': ('-', '&darr;') }
    prefix = ''
    ...
    for url in urlserver['regex'].findall(message):
        if len(url) >= min_length:
            if urlserver_settings['msg_ignore_dup_urls'] == 'on':
                if [key for key, value in urlserver['urls'].iteritems() if value[3] == url]:
                    continue
...
if __name__ == '__main__' and import_ok:
    if weechat.register(SCRIPT_NAME, SCRIPT_AUTHOR, SCRIPT_VERSION, SCRIPT_LICENSE,
        ...
        for option, value in urlserver_settings_default.iteritems():
