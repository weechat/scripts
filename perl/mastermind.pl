...
sub mastermind_init
{
    $buffer = weechat::buffer_new("mastermind", "", "buffer_close");
    if ($buffer ne "")
    {
        weechat::buffer_set($buffer, "type", "free");
        weechat::buffer_set($buffer, "title", "mastermind.pl script - enjoy!");
        weechat::buffer_set($buffer, "key_bind_meta2-A",     "/mastermind up");
        weechat::buffer_set($buffer, "key_bind_meta2-B",     "/mastermind down");
        weechat::buffer_set($buffer, "key_bind_meta2-D",     "/mastermind left");
        weechat::buffer_set($buffer, "key_bind_meta2-C",     "/mastermind right");
        weechat::buffer_set($buffer, "key_bind_meta-N",      "/mastermind new_game");
        weechat::buffer_set($buffer, "key_bind_meta-Q",      "/mastermind set");
        weechat::buffer_set($buffer, "key_bind_meta-ctrl-M", "/mastermind check_pattern");
        ...
        #weechat::buffer_set($buffer, "key_bind_meta-Z",      "/mastermind toggle_solution");
    }
}
...
sub display_line
{
    ...
    weechat::print_y($buffer, $start_y + 2 + ($y * 2), $str);
}

sub display_all
{
    # info line
    weechat::print_y($buffer, $start_y,
    ...
    # bar on top
    weechat::print_y($buffer, $start_y + 1,
                     " ┌───".("────" x $nbx)."───".("────" x $nbx)."─┐");
    ...
    for (my $y = 0; $y < $nby; $y++)
    {
        ...
        if ($y < $nby - 1)
        {
            weechat::print_y($buffer, $start_y + 2 + ($y * 2) + 1,
                             " │   ".("    " x $nbx)." │ ".("    " x $nbx)." │");
        }
    }
    
    # bottom bar
    weechat::print_y($buffer, $start_y + 2 + (($nby - 1) * 2) + 1,
                     " └───".("────" x $nbx)."───".("────" x $nbx)."─┘");
    
    # display solution and message
    weechat::print_y($buffer, $start_y + 2 + (($nby - 1) * 2) + 3,
                     ($display_solution) ? 
